...
#define HTTPGW_PROGRESS_STEP_BYTES 		(32*1024)
// For best performance make bigger than HTTPGW_PROGRESS_STEP_BYTES
#define HTTPGW_MAX_WRITE_BYTES			(512*1024)

// Report first swift download progress after 2^layer * chunksize bytes (so 0 = report every chunk)
// Arno, 2012-02-02: the binmap::cover() calculation sometimes returns a too small value
//#define HTTPGW_FIRST_PROGRESS_BYTE_INTERVAL_AS_LAYER 	5
//#define HTTPGW_PREBUF_BYTES 							(128*1024)

#define HTTPGW_FIRST_PROGRESS_BYTE_INTERVAL_AS_LAYER 	0
#define HTTPGW_PREBUF_BYTES 							0
...
// amount at app level.
#define HTTPGW_MAX_BUFFER_BYTES			(2*1024*1024)
...
void HttpGwFirstProgressCallback (int transfer, bin_t bin) {
	...
	char binstr[32];
	dprintf("%s T%i http first progress: %s width %llu contains %d seqcomp %llu\n",tintstr(),transfer, bin.str(binstr), bin.base_length(), (int)bin.contains(bin_t(0,0)), swift::SeqComplete(transfer) );

	if (swift::SeqComplete(transfer) < HTTPGW_PREBUF_BYTES)
        return;
	...
	if (req->tosend==0) { // FIXME states
		...
		//evhttp_add_header(headers, "Content-Type", "video/3gpp" );
		...
		req->tosend = filesize;
		dprintf("%s @%i http headers sent size %lli\n",tintstr(),req->id,filesize);
		 ...
		 */
		//HttpGwSubscribeToWrite(req);
		dprintf("%s @%i http direct write\n",tintstr(),req->id);
		HttpGwLibeventMayWriteCallback(-1, 0, req->sinkevreq);
    }
}
...
bool InstallHTTPGateway (struct event_base *evbase,Address bindaddr, size_t chunk_size, double *maxspeed, char *storagedir) {
	...
	httpgw_maxspeed = (double *)malloc(2*sizeof(double));
	memcpy(httpgw_maxspeed,maxspeed,2*sizeof(double));

    if (storagedir != NULL) {
    	httpgw_storage_dir = (char *)malloc(strlen(storagedir)+1);
    	strcpy(httpgw_storage_dir,storagedir);
    }
